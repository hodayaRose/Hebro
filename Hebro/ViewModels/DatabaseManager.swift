...
class DatabaseManager: ObservableObject {
    ...
    func logoutUser(completion: @escaping (Error?) -> Void) {
        do {
            ...
            ///updates the local isLogged to false
            self.isLogged = false
            completion(nil)
        } catch {
            completion(error)
    ...
    ///updating the isLogged in firestore dataBase.
    ...
    func fetchAndStoreUserName(completion: @escaping (String?, Error?) -> Void) {
        // Check if there's a current user logged in
        guard let userID = Auth.auth().currentUser?.uid else {
            completion(nil, NSError(domain: "AuthError", code: -1, userInfo: [NSLocalizedDescriptionKey: "No user is currently logged in."]))
            return
        }
        ...
        self.db.collection("users").document(userID).getDocument { [weak self] (document, error) in
            if let error  {
                ...
                let error = NSError(domain: "FirestoreError", code: -1, userInfo: [NSLocalizedDescriptionKey: "Document does not exist or the 'name' field is missing."])
                completion(nil, error)
            }
        }
    }
    ...
    ///fetch selected avatar index for current user.
